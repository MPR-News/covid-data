p <- covid_trends_actual %>%
	...
	geom_line(size = 1) +
	scale_x_date(date_breaks = "1 week", date_labels = "%b %d", expand = expansion(mult = c(.02, .02))) +
	...
	labs(title = "Rate of change for MN <span style = 'color:#E69F00'>ICU</span> and <span style = 'color:#56B4E9'>non-ICU</span> admissions",
		 ...
		 caption = caption)
fix_ratio(p) %>% image_write(here("images/change-7day-hospitalizations-trunc.png"))
