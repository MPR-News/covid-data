...
covid_data_report_short %>%
	...
	select(new_cases, new_tests, positivity, new_hosp, new_deaths, active_cases, new_vax_onedose, new_vax_complete, new_vax_boosted) %>%
	set_names("Cases", "Tests", "Positivity rate", "Hospitalizations", "Deaths", "Active cases", "Vaccinations (1+ dose)", "Vaccinations (complete)", "Vaccinations (boosters)") %>%
	pivot_longer(everything(), values_to = "New") %>%
	# Add totals column
	left_join(covid_totals_report %>%
			  	filter(date == max(date)) %>%
			  	select(total_positives_reinfections, total_tests, total_hosp, total_deaths, total_vax_onedose, total_vax_complete, total_vax_boosted) %>%
			  	mutate(across(is.numeric, comma, accuracy = 1)) %>%
			  	set_names("Cases", "Tests", "Hospitalizations", "Deaths", "Vaccinations (1+ dose)", "Vaccinations (complete)", "Vaccinations (boosters)") %>%
	...
	left_join(covid_trends_report_short %>%
			  	...
			  	select(-day) %>%
			  	arrange(date) %>%
			  	set_names("Date", "Cases", "Tests", "Positivity rate", "Hospitalizations", "Deaths", "Active cases", "Vaccinations (1+ dose)", "Vaccinations (complete)", "Vaccinations (boosters)") %>%
			  	mutate(across(-Date, ~. - lag(.))) %>%
			  	filter(!is.na(Cases)) %>%
			  	select(-Date) %>%
			  	pivot_longer(everything()) %>%
			  	mutate(value = case_when(name == "Positivity rate" & value < 0 ~ percent(value, accuracy = .01),
			  							 name == "Positivity rate" ~ percent(value, accuracy = .01, prefix = "+"),
			  							 value < 0 ~ comma(value, accuracy = 1),
...
list(covid_data_actual %>%
	 	...
	 	filter(date == max(date)) %>%
	 	select(new_cases, new_tests, positivity, new_hosp) %>%
	 	set_names("Cases", "Tests", "Positivity rate", "Hospitalizations") %>%
	 	pivot_longer(everything(), values_to = "New"),
	 covid_trends_actual %>%
	 	filter(cases_complete == TRUE) %>%
	 	filter(date == max(date)) %>%
	 ...
	 covid_trends_actual %>%
	 	...
	 	filter(date %in% c(max(date), (max(date) - 7))) %>%
	 	arrange(date) %>%
	 	select(date, new_cases, new_tests, positivity, new_hosp) %>%
	 	set_names("Date", "Cases", "Tests", "Positivity rate", "Hospitalizations") %>%
	 	mutate(across(-Date, ~. - lag(.))) %>%
	 	filter(!is.na(Cases)) %>%
	 	select(-Date) %>%
	...
	bind_rows(list(covid_data_actual %>%
				   ...
				   covid_trends_actual %>%
				   	...
				   	pivot_longer(everything(), values_to = "Past week's change")) %>%
			  	reduce(left_join, by = "name")) %>%
	mutate(across(is.numeric, ~case_when(name == "Positivity rate" ~ percent(., accuracy = .01),
										 TRUE ~ comma(., accuracy = .01) %>% str_remove_all("\\.0*$")))) %>%
...
## Positivity rate {.tabset}

### Report date

![](../images/positivity-years.png "Graph of new Minnesota COVID-19 positivity rate by report date by year")

### Actual date

![](../images/positivity-years-sample.png "Graph of new Minnesota COVID-19 positivity rate by sample date by year")
...
![](../images/tests-years-sample.png "Graph of new COVID-19 tests in Minnesota by year")

### Rate of change

![](../images/change-14day-sample.png "Graph of the rate of change in COVID-19 metrics vs. the past week")
